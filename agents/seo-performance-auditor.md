...
6. **Your probe command:** `page <url>` — TTFB, HTML bytes, `content-encoding`,
   `cache-control`, CDN cache headers, script count and inline script weight, plus
   the rendering verdict. Pair it with the PageSpeed field data call in
   `live-site-audit.md` for real-user LCP/INP/CLS.
